    ...
    ID id;
{
	...
	var = ALLOC(struct global_variable);
        ...
        BOP_promise( &entry, sizeof( struct global_entry ) );
        BOP_promise( &var, sizeof( struct global_variable ) );
    ...
    VALUE val;
{
	...
	bop_msg(6, "val getter : %llx\n", &gvar->data);
	BOP_use( &gvar->data, sizeof( VALUE ) );
...
val_setter(val, id, data, var)
    ...
    struct global_variable *var;
{
	bop_msg(6, "val setter : %llx\n", &var->data);
	BOP_promise( &var->data, sizeof( VALUE ) );
    ...
    VALUE *var;
{
    ...
    bop_msg(6, "var getter : %llx\n", var);
	BOP_use( var, sizeof( VALUE ) );
...
var_setter(val, id, var)
    ...
    VALUE *var;
{
    bop_msg(6, "var setter :  %llx\n", var);
	BOP_promise( var, sizeof( VALUE ) );
    ...
    struct global_entry *entry;
{
    ...
    BOP_use( & var->data, sizeof(VALUE) );
    //bop_msg( 1, "get var: var->data %p, %p", &var->data, (void*) var->data);
    ...
    VALUE val;
{
    ...
    BOP_promise( &var->data, sizeof( VALUE ) );
    //bop_msg( 1, "set var var->data %p, initial val %p, new val %p", &var->data, (void*) var->data, (void*) val);
