...
heap_page_create(rb_objspace_t *objspace)
{
    ...
    if (1) bop_msg(5, "heap_page_create: %s - %p, heap page body %p, heap_allocated_pages: %d, heap_allocated_pages: %d, tomb_page_length: %d, objspace %p",
...
gc_start(rb_objspace_t *objspace, const int full_mark, const int immediate_mark, const unsigned int immediate_sweep, int reason)
{
    ...
    bop_msg(2, "Starting to collect garbage");
    if (!is_sequential()) bop_msg(2, "In Parallel");
