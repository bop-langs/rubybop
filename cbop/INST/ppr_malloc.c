...
void *BOP_pagefill_malloc(size_t sz) {
  ...
  memaddr_t obj = (memaddr_t) mspace_memalign(serial_space, PAGESIZE, PAGEFILLEDSIZE(sz));
  bop_msg(2, "BOP_pagefill_malloc %d bytes at 0x%x (pages [%llu, %llu])", sz, obj, PAGESTART(obj)>>PAGESIZEX, PAGEEND(obj+sz-1)>>PAGESIZEX);
