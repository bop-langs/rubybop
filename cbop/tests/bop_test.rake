...
# Compiler config
$cc = 'gcc' if $cc.nil?
$c_flags = '-g3' if $c_flags.nil?
$ldflags = '-lm -Wall' if $ld_flags.nil?

# Location of BOP
$bop_dir = (Pathname.new(__FILE__).dirname + '../src').cleanpath if $bop_dir.nil?
...
def bop_test(name, sources = nil, orig = true)
  ...
  if orig
    ...
    file orig_prog => objs do
      sh "#{$cc} #{$ldflags} -o #{orig_prog} #{objs * ' '}"
    end
  end
  ...
  file bop_prog => bop_objs + [:boplib] do
    sh "#{$cc} #{$ldflags} -o #{bop_prog} #{bop_objs * ' '} #{$bop_lib}"
  end
end
...
rule( bop_o => proc {|n| n.sub(bop_o, '\1\3.c')} ) do |t|
  ...
  sh "#{$cc} #{bop_flags} #{$c_flags} -I#{$bop_dir} -c -o #{t.name} #{t.source}" 
end
